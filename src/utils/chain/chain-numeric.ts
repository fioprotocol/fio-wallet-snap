...
export function isNegative(bignum: Uint8Array | undefined) {
  return !!bignum && bignum.length > 0 && (bignum[bignum.length - 1]! & 0x80) !== 0;
}
...
function stringToKey(s: string, type: KeyType, size: number, suffix: string): Key {
  ...
  const dataBuffer = digestSuffixRipemd160(result.data, suffix);
  const dataToBuffer = Buffer.isBuffer(dataBuffer) ? dataBuffer : Buffer.from(dataBuffer);
  const digest = new Uint8Array(dataToBuffer);
...
function keyToString(key: Key, suffix: string, prefix: string) {
  const keyDataBuffer = digestSuffixRipemd160(key.data, suffix);
  const keyDataToBuffer = Buffer.isBuffer(keyDataBuffer) ? keyDataBuffer : Buffer.from(keyDataBuffer);
  const digest = new Uint8Array(keyDataToBuffer);
...
export function stringToPublicKey(s: string): Key {
  ...
  if (s.substring(0, 3) === FIO_CHAIN_NAME) {
    ...
    const dataBuffer = ripemd160(key.data as Buffer);
    const dataToBuffer = Buffer.isBuffer(dataBuffer) ? dataBuffer : Buffer.from(dataBuffer);
    const digest = new Uint8Array(dataToBuffer);
